...
$graphSPId = "71812eae-163b-4c6d-9b63-99d7de686976" # Microsoft Graph SP id
...
#$spResponse = Invoke-MgGraphRequest -Method GET -Uri "https://graph.microsoft.com/v1.0/servicePrincipals?$filter=appId eq '$ApplicationID'" -Headers @{ "…
 
#$ServicePrincipalId = $spResponse.value[0].id
#Write-Host $ServicePrincipalId
 
 Write-Host "App $AppName"
# Get apllication object id and App id using app display name
...
$fetchAppname = $appResponse.value[0].displayName
write-host "fetched Appname is $fetchAppname"
...
$spResponse = Invoke-MgGraphRequest -Method GET -Uri "https://graph.microsoft.com/v1.0/servicePrincipals(appId=`'$ApplicationID`')" -Headers @{ "Content-Type" = "application/json" }
$ServicePrincipalId = $spResponse.id
Write-Host "*********Debug ServicePrincipal object ID: $ServicePrincipalId ***********"
...
# Using Beta endpoint , expiryTime is mandatory
<#
$consentBody = @{
...
$consentBody = @{
 ...
 Write-Host "consent body $consentBody"
